...
export function initAirlandPage() {
  // 1. Insere os links para os CSS externos no <head> (caso ainda não existam)
  const cssFiles = [
  ...
  // 2. Insere o CSS customizado exatamente como você enviou
  // (sem nenhuma alteração)
  if (!document.querySelector("#custom-arrow-style")) {
    const style = document.createElement("style");
    style.id = "custom-arrow-style";
    style.textContent = `
    /* Exemplo de estilização da seta (seta “→” antes do texto) */
    .arrow-icon {
      margin-right: 6px; /* Ajuste conforme quiser */
      color: #ffffff;    /* Ajuste a cor conforme quiser */
    }
    `;
    document.head.appendChild(style);
  }

  // 3. Define o conteúdo HTML exatamente conforme seu layout original.
  // Como não podemos inserir as tags <html>, <head> e <body> (já existentes),
  // vamos injetar o conteúdo a partir do preloader.
  const htmlContent = `
    <!-- ======== Preloader =========== -->
    <div id="preloader">
  ...
  // 4. Substitui o conteúdo atual do <body> pelo HTML definido
  ...
  // 6. Busca os dados do usuário a partir do endpoint e atualiza os placeholders
